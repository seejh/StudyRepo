...
1. 프로세스 동작 중에 인터럽트 발생 -> 커널 개입하여 프로세스 중단, 현재 컨텍스트를 PCB에 저장.
2. 커널 인터럽트 핸들러에서 인터럽트 핸들링(원인과 발생지를 파악하고 그에 맞는 인터럽트 서비스 루틴 찾아서 실행)
3. 

현재 프로세스가 프로세서를 할당 받아 동작 중에 인터럽트 발생 -> 커널 개입하여 프로세스 중단 -> 
프로세스의 현재 컨텍스트를 PCB에 저장 -> 
이후 커널의 인터럽트 핸들러가 인터럽트 핸들링(어디서 발생? 원인은 뭔가 -> 올바른 조치는?)
