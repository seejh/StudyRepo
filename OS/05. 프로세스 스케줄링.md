...
작업 중에 더 급한 작업이 생기더라도 현재 작업을 주어진 시간 동안 끝까지 작업한다는 말로
컨텍스트 스위칭의 오버헤드가 적다는 장점이 있지만 잦은 우선 순위 역전, 평균 응답 시간 증가의 단점이 있다.
...
작업 중에 더 급한 작업이 오면 해당 작업 먼저 처리.<br/>
응답성이 높아지지만 그만큼 컨텍스트 스위칭의 오버헤드가 커진다.<br/>
