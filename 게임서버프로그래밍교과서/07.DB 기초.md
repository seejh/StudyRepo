...
이를 OwnerUserAccountID 필드는 UserAccount의 ID 필드를 가리키는 ___외래키___ 라고 한다.
<br/><br/>
외래키를 가지고 CRUD를 자주 한다. 따라서 외래 키에 대해 인덱스를 설정하는 것이 좋다. 다만 여기에서 외래 키는 중복을 허락해야 하므로
논유니크 인덱스(nonunique index)로 설정해야 한다. 



${\textsf{\color{green}하하하하하하하핳}}$
