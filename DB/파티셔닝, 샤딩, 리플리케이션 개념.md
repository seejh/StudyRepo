...
**Column**을 기준으로 테이블을 나누는 방식으로 정규화도 vertical 방식이다. <br/>
<img width="510" height="309" alt="image" src="https://github.com/user-attachments/assets/f919fc94-b221-4f2a-8201-a4449d1c8e53" /><br/>
위의 그림을 보면 하나의 테이블을 칼럼을 기준으로 나누고 있다. <br/>

#### 정규화란?
* db에 중복된 데이터가 저장되는 것을 방지하기 위한 목적
* insert, update, delete를 할 때 문제가 생기는 것을 방지하기 위한 목적

#### vertical 방식의 필요성
아래는 게시판에 기재되는 게시물(Article)에 대한 테이블과 해당 테이블을 실사용하는 모습이다. <br/>
<img width="700" height="200" alt="image" src="https://github.com/user-attachments/assets/a8ba3421-a63f-4571-9212-48a3fd3a114f" /><br/>
<img width="600" height="400" alt="image" src="https://github.com/user-attachments/assets/38094444-3683-466a-86bd-88107ca1c878" /><br/>
...
Content는 게시물의 내용에 대한 항목으로 위의 상황에서는 필요하지 않고 또한 데이터의 사이즈가 매우 클 것이라고 예상이 된다. 그래서 쿼리에서도
Content는 포함시키지 않고 SELECT로 조회를 한다. 그러나 위의 실제 동작 과정을 보면 Content의 내용까지 들고 오고 그 후에 필터링을
...
유의 사항에 대해서 알아보자. <br/>
dingyo가 구독한 채널들 정보를 모두 조회하고 싶다면?
dingyo라는 유저의 구분이 user_id로 되고 파티션 키가 user_id이므로 한 파티션에서 모든 조회를 할 수 있다.

id가 1인 채널을 구독한 사용자의 id를 모두 조회하고 싶다면?
모든 테이블을 다 조회하여야 한다.

고로 가장 많이 사용될 패턴에 따라 파티션 키를 정하는 것이 중요하다.

데이터가 균등하게 분배될 수 있도록 hash function을 잘 정의하는 것도 중요하다

해시 기반으로 파티셔닝을 나누게 되면 이후 유지보수(파티션 추가 또는 제거)를 하기가 까다롭기에 한 번 설정할 때 잘 고민해서 해야 한다.
