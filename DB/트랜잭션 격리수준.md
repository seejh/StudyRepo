...
# 트랜잭션 테스트
## 테스트1
SP_LOCK, SP_WHO 사용해서 락이 걸린 상황을 체크한다. <br/>
아래 세 가지 중 하나를 만족하면 해당 세션은 현재 배타 락을 걸고 있는 세션이다. <br/>
* SP_LOCK으로 확인했을 때 배타 잠금(X)이 걸린 세션
* SP_LOCK으로 확인했을 때 BlkBy(Blocked By) 칼럼에 값이 있는 세션
* SP_WHO로 확인햇을 때 blocked 칼럼에 값이 있는 세션
...
DB세션1(spid-64)이 현재 배타 락을 걸고 있다. <br/>

SP_WHO <br/>
![sp_who](https://github.com/user-attachments/assets/f611b318-22dc-44db-aaaf-5d69cb23d003) <br/>
DB세션2(spid-62)가 DB세션1(spid-64)에 의해 Block 당해 Suspend하고 있다. <br/>
