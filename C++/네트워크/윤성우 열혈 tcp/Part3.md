...
### 18-2 윈도우 기반의 스레드 생성
윈도우는 프로그램의 실행 단위가 "스레드"이다. 프로그램을 실행한다고 할 때 윈도우는 프로세스를 생성 후 프로세스 내부에 메인 스레드라는 것을 생성한다.
이 메인 스레드가 main 함수를 실행한다. 프로그램 시작점인 main을 실행하는 것이 프로세스가 아니라 프로세스 내부에 존재하는 스레드이며 프로세스는 프로그램을
실행시키는 일의 단위가 아니라 스레드를 담고 있는 저장소라 볼 수 있다. 이 점이 윈도우와 유닉스 계열의 차이점이다.<br/>

'''c++
// 이 예제는 메인 스레드가 끝나면 프로세스가 종료된다는 것을 알려준다.
// 추가 스레드가 끝나기 전에 메인이 끝나 프로세스가 종료되는 것을 방지하기 위해
// 메인 스레드에서 다른 스레드를 기다리게 만든다.
...
int main() {
  ...
  // 스레드에서 호출할 함수(3번째), 해당 함수에 전달할 인자(4번째)
...
정리<br/>
스레드는 커널에 의해 생성되고 관리되는 리소스. 스레드 생성 함수를 호출하면 커널은 해당 스레드를 관리하기 위한 커널 오브젝트를 생성한다.
프로그램상에서 해당 커널 오브젝트를 구분하고 사용하려면 핸들을 발급 받아 사용한다.<br/>
...
이중 모드 연산(Dual-Mode Operation). 운영체제가 시스템을 보호하기 위해 cpu의 연산 방식을 사용자 모드와 커널 모드, 두 가지로
나누어 실행하는 것을 말한다. 프로그램을 실행시키면 유저 모드와 커널 모드를 오가며 프로그램을 실행하게 된다. 시스템을 보호? 
예를 들어 내가 만든 프로그램이 잘못해서 다른 프로그램의 메모리에 접근해서 이상한 연산을 해버리면 큰일일 것이다. 
...
커널 모드<br/>
스레드, 파일등은 커널 오브젝트라고 하며 이러한 시스템 리소스에 접근은 유저 모드에서 할 수 없다. 하지만 우리는 앞에서 
이 둘을 만들고 사용했다. 어떻게 된 것일까?<br/>

모드 전환이 일어나기 떄문. 프로그램이 실행되는 과정에서 공유되는 시스템 자원에 접근하는 경우, 프로그램 레벨에서 직접 접근하는 것이 아니라 커널 모드로
...
커널 모드로의 전환이 일어나 시스템에 의해 처리된 후 다시 유저 모드로 변환되어 프로그램을 실행한다.<br/>

유저 모드 동기화 (CRITICAL_SECTION)<br/>
...
유저 모드 동기화를 사용하면 이런 변환이 필요없어 부하를 줄일 수 있으나 커널 모드에 비해 제한된 기능만을 갖는다.<br/>

커널 모드 동기화 (Event, Semaphore, Mutex)<br/>
...
데드락<br/>
데드락은 다음과 같다. a가 lock1을 잡고 lock2를 요구하는 상황에서 b가 lock2를 잡은 채로 lock1을 요구하는 상황.
...
커널 모드 동기화(Mutex 커널 오브젝트를 사용한)<br/>
크리티컬 섹션과 비슷하지만 얘는 커널 오브젝트<br/>

Mutex 오브젝트는 소유자가 존재하는 경우 non-signaled, 그 반대는 signaled가 된다.<br/>
WaitForSingleObject 함수는 커널 오브젝트가 non-signaled 상태에서 signaled가 되기를 기다리는 함수이다.<br/>
Mutex 오브젝트를 WaitForSingleObject를 하면 Mutex 소유자가 존재하지 않는 것(signaled)을 기다리는 것<br/>
<img width="742" height="339" alt="image" src="https://github.com/user-attachments/assets/18b63b7e-bce3-4ae5-af90-56cca51e4ba0" /><br/>
뮤텍스 생성 -> 소유자 없음 -> non-signaled -> A 스레드 WaitForSingleObject 호출 후 소유 -> 임계 영역 진입 -> 
B 스레드 WaitForSingleObject 호출, 스레드 A가 소유중으로 non-signaled -> 대기 -> A 스레드 뮤텍스 반납 -> B 진입

```c++
// Critical Section에서 최대한 차이나는 부분만 작성

// 커널 오브젝트라 handle 사용
HANDLE hMutex;

void ThreadTask() {
  WaitForSingleObject(hMutex, INFINITE); // 뮤텍스 획득 시도
...
윈도우에서 제공하는 확장된 입.출력 모델
### 20-1 비동기 Notification?
### 20-2 WSAEventSelect 모델 기반 서버 구현을 위한 API
### 20-3 WSAEventSelect 모델 기반의 에코 서버 구현
