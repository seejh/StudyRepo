...
JWT 토큰 발행
레디스에 JWT 토큰 삽입
클라이언트에게 JWT 토큰, 서버 현황 응답
2. 게임 클라에서 JWT 토큰을 가지고 게임 서버로 접속
서버 현황을 띄우고 접속할 서버를 선택하게 함
선택한 서버로 소켓 연결
JWT 토큰 패킷으로 보냄

검증
레디스를 열어서 해당 토큰이 있는지 확인

서버 측에서 레디스 확인하여 해당 토큰이 있는지 확인, 및 검증


// 1. 
jwt토큰을 쓰는데 왜 레디스가 필요한지에 대한 이해 필요

// 2. 
현재 상황이 jwt 발급이랑 레디스에 넣는것 까진 OK
다만 해당 토큰 검증 및 인가는 어떻게 처리하는지 확인 필요
